      ...
      <div className="container lg:flex">
        ...
        <div className="lg:ml-auto xl:w-[38rem] mt-4">
          ...
          <div className="relative left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale:75 md:scale-100">
            ...
            <ul>
              {collabApps.map((i, index) => (
                <li key={i.id} className={`absolute top-0 left-1/2 h-1/2 -ml-[1.6rem] origin-bottom rotate-${index * 45}`}>
                  <div className={`relative -top-[1.6rem] flex w-[3.2rem] h-[3.2rem] bg-n-7 border border-n-1/15 rounded-xl -rotate-${   index * 45 }`} >
                    <img className="m-auto" width={i.width} height={i.height} src={i.icon} />
                  </div>
                </li>
              ))}
            </ul>
